...
- **Daily headless smoke workflow — `.github/workflows/headless-smoke.yml`** (Epic #484, #501): New GitHub Actions workflow that runs every day at UTC 06:00 and on-demand via `workflow_dispatch`. The first job, `safari-smoke`, boots an iPhone simulator on `macos-latest`, starts `ios-webkit-debug-proxy`, opens Safari via `simctl openurl`, and runs `opensafari audit` with JUnit output — entirely with `OPENSAFARI_HEADLESS_ONLY=1` and without `OPENSAFARI_ALLOW_FOCUS_INPUT`. JUnit report is uploaded as an artifact; CoreSimulator system logs are uploaded on failure for triage. Flutter, native, and WebView smoke jobs will land in follow-up PRs targeting the same workflow file.
- **Flutter headless smoke job** (Epic #484, #501): Adds `flutter-smoke` job to `.github/workflows/headless-smoke.yml`. Installs Flutter via `subosito/flutter-action`, boots an iPhone simulator, launches the `tests/integration/fixtures/flutter_sample` fixture via `flutter run --machine` (DDS required for Dart `evaluate` RPC), and runs `tests/integration/flutter-vm-input.live.test.ts` with `OPENSAFARI_LIVE_VM=1`. Exercises `FlutterVMInputBackend` end-to-end on CI. Emits JUnit via `jest-junit`, uploads flutter-run logs on any run, and grabs a simulator screenshot on failure. Asserts the audit log is free of AppleScript fallback references. Native and WebView smoke jobs will land in follow-up PRs targeting the same workflow file.
- **Native (SimulatorKit HID) headless smoke job** (Epic #484, #501): Adds `native-smoke` job to `.github/workflows/headless-smoke.yml`. Builds `dist/sim-hid-bridge`, boots an iPhone simulator, and runs the `backend selection`, `headless characteristics`, `hardware buttons`, and `performance` describes from `tests/integration/sim-hid-input.live.test.ts`. Confirms the Tier-1 `SimulatorKitHIDInputBackend` is selected without `OPENSAFARI_ALLOW_FOCUS_INPUT`, that the physical mouse cursor never moves while events are dispatched, and that Simulator.app is never brought forward. The `Settings.app automation` describe is skipped pending the iOS 26 UIKit mouse-HID regression tracked inline in the test file. WebView smoke job will land in a follow-up PR.
