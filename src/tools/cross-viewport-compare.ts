      ...
      if (params.format === 'html') {
        ...
        const deviceSections = captures
          .map((cap: any) => {
            ...
            return `<div style="flex:1;min-width:280px;"><h3 style="font-size:14px;font-weight:600;margin-bottom:8px;">${cap.device ?? 'Unknown'} (${cap.viewport?.w ?? '?'}x${cap.viewport?.h ?? '?'})</h3>${screenshotImg}</div>`;
          })
          .join('');

        const html = `<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>Cross-Viewport Comparison - ${params.url}</title>
...
<p style="color:#6b7280;margin-bottom:24px;">URL: ${params.url}</p>
